(function () {
  ...
  function AppController(GithubService) {
    ...
    function init() {
      GithubService.query('/users/menico').then(function (response) {
        vm.user = response.data;
        console.log(vm.user);
      });

      GithubService.query('/users/menico/starred').then(function (response) {
        ...
        vm.languages = (vm.starred || [])
          ...
          .filter(function (item, pos, self) {
            if (item && self.indexOf(item) == pos) return item;
          });
